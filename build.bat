...
"%GCC%" -shared -O2 -std=c++17 -static-libgcc -static-libstdc++ -s -Iinclude -Iinclude\win32 -o MCCombatStatusJni.dll src\MCCombatStatusJni.cpp -lws2_32
...
if errorlevel 1 goto :err
echo.
echo [OK] done: MCCombatStatusJni.dll + injector.exe
exit /b 0
:err
echo [!!] build failed
exit /b 1
